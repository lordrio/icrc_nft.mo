...
shared (_init_msg) actor class Example(
  ...
  stable var icrc7_migration_state = ICRC7.init(
    ...
    switch (_args.icrc7_args) {
      case (null) ICRC7Default.defaultConfig(boss_principal);
      case (?val) val;
    },
    boss_principal,
  );
  ...
  stable var icrc37_migration_state = ICRC37.init(
    ...
    switch (_args.icrc37_args) {
      case (null) ICRC37Default.defaultConfig(boss_principal);
      case (?val) val;
    },
    boss_principal,
  );
  ...
  stable var icrc3_migration_state = ICRC3.init(
    ...
    switch (_args.icrc3_args) {
      case (null) ICRC3Default.defaultConfig(boss_principal);
  ...
  // public shared (msg) func icrcX_burn_owner(target : Principal, tokens : ICRC7.BurnNFTRequest) : async ICRC7.BurnNFTBatchResponse {
  //   if (msg.caller != icrc7().get_state().owner) {
  //     D.trap("unauthorized");
  //   };

  //   switch (icrc7().burn_nfts<system>(target, tokens)) {
  //     case (#ok(val)) val;
  //     case (#err(err)) D.trap(err);
  //   };
  // };
